...
TOPIC_REQUEST = "simulate/telemetry"         # 백엔드로 request 데이터 보내기
...
def load_csv_data():
    global all_csv_data
    try:
        with open("Sim_Live_Data_10m_Compressed_Rounded.csv", "r") as f:
            reader = csv.DictReader(f)
            for row in reader:
    ...
    while is_running:
        ...
        if current_step == 22 and sec_idx == 0 and current_day_idx in sim_daily_requests:
            try:
                ...
                aws_client.publish(TOPIC_REQUEST, json.dumps(ai_request_payload))
                ...
                print(f"💾 [파일 저장] sample_ai_server_request.json 파일을 업데이트했습니다.")
            except Exception as e:
